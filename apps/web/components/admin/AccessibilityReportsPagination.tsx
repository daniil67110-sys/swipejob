...
export type AccessibilityReportsPaginationProps = {
  page: number;
  totalPages: number;
  total: number;
  pageSize: number;
  /** Search params à conserver dans les liens (status). */
...
function buildHref(preserved: Record<string, string | undefined>, page: number): string {
  ...
  return qs ? `/admin/signalements?${qs}` : '/admin/signalements';
}

export function AccessibilityReportsPagination({
  page,
  totalPages,
  total,
  pageSize,
  preserved,
}: AccessibilityReportsPaginationProps) {
  if (totalPages <= 1) {
    return (
